document.addEventListener('DOMContentLoaded', () => {
  ...
  function mettreAJourGraphiques() {
    fetch('/api/data')
      ...
      .then(data => {
        ...
        creerGraphique('bruitChart', 'Bruit (dB)', 'bruit', 'rgba(75, 192, 192, 1)', data);
        creerGraphique('noxChart', 'NOx (ppm)', 'nox', 'rgba(153, 102, 255, 1)', data);
        creerGraphique('co2Chart', 'CO₂ (ppm)', 'co2', 'rgba(255, 159, 64, 1)', data);
        creerGraphique('uvChart', 'Index UV', 'uv', 'rgba(255, 205, 86, 1)', data);
      })
      .catch(() => {
  ...
  function afficherCarte(data) {
    const map = L.map('mapid').setView([50.845693, 4.393563], 13); // Coordonées par défaut si aucune donnée
