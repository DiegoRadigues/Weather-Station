...
app.post('/api/data', (req, res) => {
  ...
  const { temperatureBMP, pression, humidite, gps } = req.body;

  const isValidNumber = (value) => typeof value === 'number' && !isNaN(value);
  const isValidString = (value) => typeof value === 'string' && value.trim() !== '';

  // Vérification individuelle des champs, mais pas de rejet automatique si une donnée est manquante
  const errors = [];
  if (!isValidNumber(temperatureBMP)) {
    console.warn("Donnée invalide ou absente : temperatureBMP");
    errors.push("temperatureBMP");
  }
  if (!isValidNumber(pression)) {
    console.warn("Donnée invalide ou absente : pression");
    errors.push("pression");
  }
  if (!isValidNumber(humidite)) {
    console.warn("Donnée invalide ou absente : humidite");
    errors.push("humidite");
  }
  if (!isValidString(gps)) {
    console.warn("Donnée invalide ou absente : gps");
    errors.push("gps");
  }
  ...
  fs.readFile(dataFilePath, 'utf8', (err, data) => {
    ...
    // Ajout des nouvelles données avec des valeurs par défaut pour les champs manquants
    const newData = {
