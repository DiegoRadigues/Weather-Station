...
void loop() {
  ...
  // Met à jour l'intervalle toutes les 60 secondes
  if (tempsActuel % 60000 == 0) {
    mettreAJourIntervalle();
  }

  delay(1000); // Réduit la charge du microcontrôleur
}
...
void envoyerDonnees() {
  ...
  postData += "\"bruit\":" + (isnan(bruit) ? "null" : String(bruit)) + ",";
  postData += "\"nox\":" + (isnan(nox) ? "null" : String(nox)) + ",";
  postData += "\"co2\":" + (isnan(co2) ? "null" : String(co2)) + ",";
...
float lireBruit() {
  return 50.0; // Simulation d'une lecture de bruit
}

float lireNOx() {
  return 0.05; // Simulation d'une lecture de NOx
}

float lireCO2() {
  return 400.0; // Simulation d'une lecture de CO2
}


String lireGPS() {
  if (capteurGPSDispo) {
    // Simulation de la lecture du GPS
    return "50.845536242342234, 4.393515690208581"; 
  } else {
    return "";
  }
}
...
float lireUV() {
  if (capteurUVDispo) {
    // Simulation de la lecture du capteur UV
    return 5.0;
  } else {
    return NAN;
  }
